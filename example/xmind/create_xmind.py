...
def gen_my_xmind_file():
    ...
    xmind.save(workbook, path='test2.xmind')
...
def design_sheet1(sheet1):
    ...
    root_topic1.setTitle("root node2")  # set its title

    # create some sub topic element
    sub_topic1 = root_topic1.addSubTopic()
    sub_topic1.setTitle("first sub topic")

    root_topic1.addSubTopic().setTitle("second sub topic")
    root_topic1.addSubTopic().setTitle("third sub topic")
    root_topic1.addSubTopic().setTitle("fourth sub topic")

    # create a detached topic(attention: only root topic can add a detached topic)
    root_topic1.addSubTopic(topics_type=TOPIC_DETACHED).setTitle("detached topic").setPosition(0, 30)
